        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            for (int i = 0; i < maxRetries; i++)
            {
                ...
                //     "Starting gratuity attempt {Attempt}/{MaxRetries} for ticket {TicketId}",
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    try
                    {
                        ...
                        // _logger.LogInformation("Fresh scope created for attempt {Attempt}. Resolving ticket...", i + 1);
                        ...
                        // _logger.LogInformation(
                        //     "Ticket loaded - ID: {TicketId}, Version: {Version}, Status: {Status}, HasGratuity: {HasGratuity}",
                        ...
                        // _logger.LogInformation("Gratuity applied in memory. Attempting to save...");
                    ...
                    {
                        _logger.LogWarning(
                            "Concurrency exception caught on attempt {Attempt}/{MaxRetries}. Exception: {Message}",
                            i + 1, maxRetries, ex.Message);

                        if (i == maxRetries - 1)
                        {
                            _logger.LogError(ex, 
                                "Max retries reached. Concurrency conflict applying gratuity to ticket {TicketId}.", 
                                command.TicketId);
                            throw; 
                        }

                        _logger.LogWarning(
                            "Will retry... Disposing current scope and creating fresh DbContext for attempt {NextAttempt}",
                            i + 2);
